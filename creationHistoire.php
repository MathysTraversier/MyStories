...
<body>
    ...
    if (isset($_GET['creation']) || $_SESSION['creation'] == 3) {
        $_SESSION['creation'] = 1;
    }
    ...
    ?>
        <div class="container mt-5 mb-5">
            <div id="pageCreation" class="card page p-5">
                <h3 class="titrePage text-center">SAISIR UNE NOUVELLE HISTOIRE</h3>
                <form method="POST" action="ajoutParagraphesBDD.php" enctype="multipart/form-data">
                    <?php
                    if (isset($_GET['error']) && $_GET['error'] == 1) {
                    ?>
                        <div class="form-outline w-75 mx-auto mb-2 text-center text-danger">
                    ...
                    if (isset($_SESSION['nbParagraphes'])) {
                        ...
                        ?>
                            <div class="mt-5">
                                ...
                                <textarea class="form-control" name="paragraphe<?= $i ?>" rows="3" cols="33" required></textarea><br />
                                <input type="checkbox" name="start<?= $i ?>" />
                                <label class="form-label" for="start<?= $i ?>">Perte de PV</label><br />
                                <input type="checkbox" name="lossPV<?= $i ?>" />
                                <label class="form-label" for="lossPV<?= $i ?>">Perte de PV</label><br />
                                <input type="checkbox" name="storyEnd<?= $i ?>" />
                                <label class="form-label" for="storyEnd<?= $i ?>">Fin de l'histoire</label><br />
                                <input type="checkbox" name="victory<?= $i ?>" />
                                <label class="form-label" for="storyEnd<?= $i ?>">Victoire</label>
                            </div>
                    <?php
                        }
                    }
